...
class UserStatusCollection:
    ...
    def search_all_status_updates(self, user_id):
        """
        Looks for all status updates for a given user_id.
        Returns an iterator.
        """
        try:
            query = StatusModel.select().where(StatusModel.user_id == user_id).iterator()
            return query
        except DoesNotExist:
            logger.debug("No status updates found for user_id %s", user_id)
            return False

    def filter_status_by_string(self, phrase):
        """
        :param: phrase: string query
        :return: all status messages matching phrase
        """
        query = (StatusModel
                 .select()
                 .where(StatusModel.status_text.contains(phrase.lower()))
                 .iterator())
        return query
