...
def search_all_status_updates(user_id, status_collection):
    """
    Retrieves all status updates for a given user_id using the status_collection.
    """
    return status_collection.search_all_status_updates(user_id)


def status_generator(status_iterator):
    """
    Generator to yield status updates one by one.
    """
    yield from status_iterator
